...
export const POST = async ({ request, cookies }: { request: Request; cookies: any }) => {
  ...
  if (!db || !event_id || !item) return new Response(JSON.stringify({ error: "missing" }), { status: 400 });
  try {
    await db.prepare("INSERT INTO gear_claims (event_id, email, item) VALUES (?,?,?) ON CONFLICT DO NOTHING").bind(event_id, user.email, item).run();
